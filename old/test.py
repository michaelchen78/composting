...
greenLayerCarbonMass = 2.073

greenLayerNitrogenMass = 0.105

greenLayerTotalMass = 4
...
def calculator(greenLayerTotalMass, greenLayerCarbonMass, greenLayerNitrogenMass, totalWaterMass, leavesPossessed,
               trimmingsPossessed, paperPossessed, cardboardPossessed, upperRange, lowerRange):
    idealMoisture = 0.6

    # these are just for range in case nothing is found

    # it's negative since its a plus below

    # make a class for brown materials
    class material:

        def __init__(self, name, moisturePercentage, carbonPercentage, nitrogenPercentage, aeration, biodegrad,
                     amountValue, possessed):
            # these are like my for big balls equation stuff
            self.name = name
            self.mass = 1  # BIG THING WRITE FO ? ?
            self.waterMass = 0
    ...
    # PaperMass': 16, 'CardboardMass': 8, 'LeavesMass': 5, 'TrimmingsMass': 8

    leaves = material(
        name="Leaves",
        moisturePercentage=0.38,  # DROP THIS BY HOW MUCH ???????????
        carbonPercentage=0.40,
        nitrogenPercentage=0.005,
        aeration=2,
        biodegrad=2,
        amountValue=6,
        possessed=leavesPossessed)
    ...
    materials = [leaves, paper, cardboard]
 ...
 \
    ...
    while done == False:
        ...
        if materialsDictionary["material0"].counter == 100:
            ...
            # print("the iteration that is printed below should be a max")
            # never mind it triggers when it is at 100 above....wait but doesnt that it make it bad lmao maybe mass lmao no matter or yes matter but
            # so in fact here is a min since first mass goes down to a 1

            # the first min
            materialsDictionary["material0"].minSwitch = 1
            materialsDictionary["material0"].minCounter += 1
            ...
            materialsDictionary["material0"].minSwitch = 0
        ...
        for i in range(len(materialsDictionary) - 2):
            ...
            if materialsDictionary["material{0}".format(i + 1)].counter == 100:
                ...
                # second through second to last min
                materialsDictionary["material{0}".format(i + 1)].minSwitch = 1
                materialsDictionary["material{0}".format(i + 1)].minCounter += 1
                # these could be redundant because it hits oh well ig... okay but still


            else:
                materialsDictionary["material{0}".format(i + 2)].switch = 0

                materialsDictionary["material{0}".format(i + 1)].minSwitch = 0
        ...
        if materialsDictionary["material{0}".format(len(materialsDictionary) - 1)].counter == 100:
            ...
            # new tactic to end it
            done = True

            # last shouldnt have a min since it goes from 0 to 100
        ...
        # moved upper and lower range from here to top
        ...
        # ok so now here is the optimization logic my person

        # if we exceed lets just say 35 to be safe for now, skip

        # if the min starts at like 35, then love the entire program

        # if (materialsDictionary["material1"].switch == 1):
        #   print(totalCarbonMass / totalNitrogenMass)

        '''
        if (materialsDictionary["material1"].minSwitch ==1):
            print(totalCarbonMass / totalNitrogenMass)
            print('hit', materialsDictionary["material0"].mass,materialsDictionary["material1"].mass)
  
  
        if(materialsDictionary["material0"].minSwitch == 1):
            for i in range(len(materialsDictionary)):
                if (materialsDictionary["material{0}".format(i+1)].minSwitch == 1):
        '''

        '''
        if(cardboard.minSwitch == 1):
            if(leaves.minSwitch == 1):
                print("HITHITHIT")
                print(leaves.mass)
                print(cardboard.mass)
            else:
                print("hit")
                print(cardboard.mass)
        ...
        if abs(totalCarbonMass / totalNitrogenMass) <= 30 + upperRange \
            ...
            # bruh no idea why this was broken
            # ha lmao
            # other places also not reset? scary scary? have to reset it each time?
            # pattern kind of there ?

            pooPooWaterMass = totalWaterMass

            for i in range(len(materialsDictionary)):
                pooPooWaterMass = pooPooWaterMass + materialsDictionary["material{0}".format(i)].water_weight

            totalMass = greenLayerTotalMass + brownLayerTotalMass
            waterMassNeeded = ((idealMoisture) * totalMass) - pooPooWaterMass
            # work lmao ?
            # check and see more ????????????
            ...
            for i in range(len(materialsDictionary)):
                ...
                comboList["combo{0}".format(goodComboCounter)][
                    materialsDictionary["material{0}".format(i)].name + "Mass"] = materialsDictionary[
                    "material{0}".format(i)].weight
            ...
            # debugging water
            # comboList["combo{0}".format(goodComboCounter)]["brownLayerTotalMass{0}".format(goodComboCounter)] = brownLayerTotalMass
            # comboList["combo{0}".format(goodComboCounter)]["totalWaterMass{0}".format(goodComboCounter)] = totalWaterMass

            #  for i in range(len(materialsDictionary)):
            #     comboList["combo{0}".format(goodComboCounter)][materialsDictionary["material{0}".format(i)].name+"WaterMass"]= materialsDictionary["material{0}".format(i)].waterMass

            '''
            #new code for the 'optimized'
  
            print("Combo ",goodComboCounter,": \n")
  
            for i in range(len(materialsDictionary)):
                print(materialsDictionary["material{0}".format(i)].name," mass: ",comboList["combo{0}".format(goodComboCounter)][materialsDictionary["material{0}".format(i)].name+"Mass"],"\n")
  
            idealness = (1/((comboList["combo{0}".format(goodComboCounter)]["totalDiff{0}".format(goodComboCounter)])*10))
  
  
            print("How ideal the combo is (0-10): ",idealness)
            '''

            # cont but here is like basically stopping it or i mean no
        ...
        if (materialsDictionary["material0"].minSwitch == 1):
            ...
            for i in range(len(materialsDictionary)):
                # print("for loop")
                # implies we go another layer
                # tbh this doesn't need to be here
                # do x operation
                # the one before
                if (materialsDictionary["material{0}".format((i + 1))].minSwitch == 1):
                    # print('reached', i)
                    ...
                    if ((totalCarbonMass / totalNitrogenMass) > (30 + (2 * upperRange))):
                        # print(totalCarbonMass / totalNitrogenMass)
                        # print('reachedperson')
                        # could also go like more direct here like straight to i+2 and dont be a person and adjust but

                        # its just that one like 23 at the end of some long list that is bothering me now

                        # p = i - 1

                        for j in range((i + 2)):
                            # q = j - 1
                            materialsDictionary["material{0}".format(j)].counter = 99
                            materialsDictionary["material{0}".format(j)].mass = 100
                            # print('done')
            ...
            if ((totalCarbonMass / totalNitrogenMass) > (30 + (2 * upperRange))):
                materialsDictionary["material0"].mass = 100
                materialsDictionary["material0"].counter = 99
    ...
    if len(sortedDiffs) == 0:
        ...
        value = calculator(greenLayerTotalMass, greenLayerCarbonMass, greenLayerNitrogenMass, totalWaterMass,
                           leavesPossessed, trimmingsPossessed, paperPossessed, cardboardPossessed, upperRange,
                           lowerRange)
        # print("Here")
        return value
    return sortedDiffs
...
if __name__ == '__main__':
    print(calculator(greenLayerTotalMass, greenLayerCarbonMass, greenLayerNitrogenMass, totalWaterMass, 10, 10, 10, 10, u, l))
