...
greenLayerCarbonMass = 18.84

greenLayerNitrogenMass = 0.969

greenLayerTotalMass = 36.5
...
while done == False:
    ...
    print(paper.mass, " ", leaves.mass, " ", cardboard.mass)

    for i in range(len(materialsDictionary)):
        brownLayerTotalMass = brownLayerTotalMass + materialsDictionary['material{0}'.format(i)].mass
    ...
    # ok so now here is the optimization logic my bitch

    # if we exceed lets just say 35 to be safe for now, skip

    # if the min starts at like 35, then fuck the entire program
    ...
    if abs(totalCarbonMass / totalNitrogenMass) <= 30 + upperRange \
        ...
        for i in range(len(materialsDictionary)):
            pooPooWaterMass = pooPooWaterMass + materialsDictionary["material{0}".format(i)].waterMass
        ...
        for i in range(len(materialsDictionary)):
            ...
            if (materialsDictionary["material{0}".format((i + 1))].minSwitch == 1):
                ...
                if ((totalCarbonMass / totalNitrogenMass) > (30 + (2 * upperRange))):
                    # print(totalCarbonMass / totalNitrogenMass)
                    # print('reachedBITCH')
                    # could also go like more direct here like straight to i+2 and dont be a bitch and adjust but
