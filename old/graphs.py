...
materials = [leaves, cardboard, paper]
...
while done == False:
    ...
    goodTotalCombinations.append(totalCombinations)
    ...
    if (materialsDictionary["material0"].minSwitch == 1):
        ...
        for i in range(len(materialsDictionary)):
            ...
            if (materialsDictionary["material{0}".format((i + 1))].minSwitch == 1):
                ...
                if ((totalCarbonMass / totalNitrogenMass) > (30 + (2 * upperRange))):
                    ...
                    totalCombinations += (
                                (101 - materialsDictionary["material{0}".format((i + 1))].mass) * (100 ** (i + 1)))
        ...
        if ((totalCarbonMass / totalNitrogenMass) > (30 + (2 * upperRange))):
            totalCombinations += (101 - materialsDictionary["material0"].mass)
...
u = np.array(totalCombinations)
v = np.array(goodTotalCombinations)
...
def CNratio(o):
    ...
    if hello:
        indeX = goodTotalCombinations.index(o)
        Y = graphY(indeX)
        return Y
    else:
        return 0

plt.plot(rr, CNratio(rr))
plt.show()
'''
...
# plt.scatter(x, z, label="stars", color = "red", marker="*", s =30)

plt.ylim(0, 160, 20)
# plt.xlim(0,1000000,200000)
