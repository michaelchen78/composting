...
while totalCombinations < expectedTotalCombo:
    ...
    if abs(totalCarbonMass / totalNitrogenMass) <= 30 + upperRange \
        ...
        for i in range(len(materialsDictionary)):
            print(materialsDictionary["material{0}".format(i)].name, " mass: ",
                  comboList["combo{0}".format(goodComboCounter)][
                      materialsDictionary["material{0}".format(i)].name + "Mass"], "\n")

        idealness = (1 / (
                    (comboList["combo{0}".format(goodComboCounter)]["totalDiff{0}".format(goodComboCounter)]) * 10))
